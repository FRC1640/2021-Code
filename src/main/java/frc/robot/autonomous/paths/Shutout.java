...
import frc.robot.autonomous.commands.Shoot5Fast;
import frc.robot.autonomous.commands.ShootCount;
import frc.robot.autonomous.commands.ShootOnStart;
import frc.robot.autonomous.commands.ShootOnStartFast;
import frc.robot.subsystems.drive.DriveSubsystem;
import frc.robot.subsystems.indexer.IndexerSubsystem;
import frc.robot.subsystems.intake.IntakeSubsystem;
import frc.robot.subsystems.shooter.ShooterSubsystem;

public class Shutout implements PathInterface {
    ...
    public Command getAutoCommand() {
        ...
        TrajectoryConfig config = new TrajectoryConfig(4.0, 3.0)
                // Add kinematics to ensure max speed is actually obeyed
                .setKinematics(kDriveKinematics);
        config.setReversed(true);

        TrajectoryConfig config2 = new TrajectoryConfig(4.0, 3.0)
                // Add kinematics to ensure max speed is actually obeyed
                .setKinematics(kDriveKinematics);
        config2.setReversed(false);

        TrajectoryConfig config3 = new TrajectoryConfig(4.0, 3.0)
                // Add kinematics to ensure max speed is actually obeyed
                .setKinematics(kDriveKinematics);
        config2.setReversed(false);
        ...
        Trajectory exampleTrajectory2 = TrajectoryGenerator.generateTrajectory(
                ...
                // End 3 meters straight ahead of where we started, facing forward
                new Pose2d(2, 2.5, new Rotation2d((Math.toRadians(-100)))), config2);//never gonna give you up, never gonna let you down

        Trajectory exampleTrajectory3 = TrajectoryGenerator.generateTrajectory(
                // Start at the origin facing the +X direction
                new Pose2d(0, 0, new Rotation2d(0)),
                // Pass through these two interior waypoints, making an 's' curve path
                List.of(new Translation2d(0.5, -0.75)),
                // End 3 meters straight ahead of where we started, facing forward
                new Pose2d(1.5, -2.5, new Rotation2d(0)), config3);
        ...
        Command shoot = new ShootOnStartFast(shooter, indexer, intake, 4000);
        // Command wait1 = new Wait(1000);
        // Command adjust = new TurnGyro(swerve, 185);
        Command pointWheels1 = new PointWheels(swerve, 0);
        ParallelCommandGroup intakeAndDrive = new ParallelCommandGroup(new IntakeAndIndex(intake, indexer, 3200),
                swerveControllerCommand);
        ParallelCommandGroup intakeAndDrive2 = new ParallelCommandGroup(new IntakeAndIndex(intake, indexer, 2000),
                swerveControllerCommand2);
        ParallelCommandGroup intakeAndDrive3 = new ParallelCommandGroup(new IntakeAndIndex(intake, indexer, 2500),
                swerveControllerCommand3);
        Command align2 = new AlignAuton(swerve, shooter);
        Command shoot2 = new Shoot5Fast(shooter, indexer, intake, 4000);

        SequentialCommandGroup trench = new SequentialCommandGroup(align, shoot, pointWheels1, intakeAndDrive,
                intakeAndDrive2, intakeAndDrive3, align2, shoot2);
